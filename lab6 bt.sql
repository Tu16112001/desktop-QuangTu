USE master
GO
IF EXISTS (SELECT * FROM sys.databases WHERE Name='QL_Khosach')
DROP DATABASE QL_Khosach
GO
CREATE DATABASE QL_Khosach
GO
USE QL_Khosach
GO
...
INSERT INTO Sach VALUES('B002','Tin hoc co ban','Huyen Sinh','Nhap mon tin hoc','2000-01-01',3,99000,200,'Tin Hoc')
INSERT INTO Sach VALUES('B003','IT khong kho','Van Dinh','IT Easy','2008-01-01',2,179000,200,'IT')
INSERT INTO Sach VALUES('B004','JS co ban','Khanh Dang','JS & REACTJS','2012-01-01',1,200000,50,'IT')
INSERT INTO Sach VALUES('B005','ky nang song','Tu Tran','1000 cau hoi vi sao','2001-01-01',3,50000,1000,'Ky nang')
...
SELECT TS,GB 
FROM Sach 
WHERE GB = (SELECT MAX(GB) FROM sach)
--11. Tìm cuốn sách có số lượng lớn nhất trong kho
SELECT TS,SL 
FROM Sach
WHERE SL = (SELECT MAX(SL) FROM sach)
...
SELECT TNXB, SUM(SL) AS 'Tong so sach'
FROM Sach
JOIN NXB ON
Sach.MS = NXB.MS
GROUP BY TNXB
